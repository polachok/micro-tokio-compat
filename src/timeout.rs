...
impl<T> Timeout<T> {
    ...
    pub fn new(value: T, timeout: Duration) -> Timeout<T> {
        ...
        Timeout::new_with_delay(value, delay, timeout)
    }

    pub(crate) fn new_with_delay(value: T, delay: Delay, timeout: Duration) -> Timeout<T> {
        Timeout { value, delay, timeout }
    }
...
impl<T> Error<T> {
    ...
    /// implementation
    pub fn timer(err: tokio_timer_02::Error) -> Error<T> {
    ...
    pub fn into_timer(self) -> Option<tokio_timer_02::Error> {
        match self.0 {
